...
const INTERIOR_LEVELS: Array[String] = ["house", "museum", "bar", "shop", "tent", "credits", "final_scene"]
...
func load_level(level: String, gate: String = "") -> void:
	...
	match level:
		...
		"credits":
			current_level = CREDITS.instantiate()
		"final_scene":
			current_level = FINAL_SCENE.instantiate()
		_:
...
func load_player(gate: String = "") -> void:
	...
	var ysort = current_level.get_node_or_null("ObjectsParent")
	if ysort:
